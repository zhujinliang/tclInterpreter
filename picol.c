...
/* 通过查找变量的名字来寻找变量，得到变量的值，返回变量结构体指针*/
...
/* 先查找变量，若找到则复制值，若否则新开辟空间，存储变量及其值*/
int picolSetVar(struct picolInterp *i, char *name, char *val) {
    ...
    if (v) {
        ...
        v->name = strdup(name);
        v->val = strdup(val);
        v->next = i->callframe->vars;  /* 同样，链表是往上增长的vars指向的永远是表头*/
        i->callframe->vars = v;
    }
    return PICOL_OK;
}
...
/* 把命令的名字，实现函数，数据存储到picolInterp链表中*/
int picolRegisterCommand(struct picolInterp *i, char *name, picolCmdFunc f, void *privdata) {
    ...
    c->func = f;
    c->privdata = privdata;
    c->next = i->commands;  /* 注意此处链表是往上增长的*/
    i->commands = c;
    return PICOL_OK;
}
        ...
        if (p.type == PT_EOL) 
        {
            ...
            if (argc) 
            {
                ...
                {  /* 查找命令*/
                    ...
                    else if ((picolGetVar(i, argv[0]) != NULL) && (argv[1][0] == '('))
                    {
                        c = picolGetCommand(i, "get");
                    }
                    else if ((argc > 2) &&  (c = picolGetCommand(i, argv[1])))
                    {
                        //Debugf("operate\n");//math operation, the operator is in the middle.
                    }
                    else
                    {
...
int picolCommandMath(struct picolInterp *i, int argc, char **argv, void *pd) {
    ...
    {
        a = atoi(argv[1]);
        if (argv[0][0] == '!' && argv[0][1] == '\0') c = (!a);
        else if (argv[0][0] == '~' && argv[0][1] == '\0') c = (~a);
        else return picolArityErr(i,argv[0]);
    }
    else if (argc != 3) 
    {
    ...
    b = atoi(argv[2]);
    if (argv[1][0] == '+') c = a+b;
    else if (argv[1][0] == '-') c = a-b;
    else if (argv[1][0] == '*') c = a*b;
    else if (argv[1][0] == '/') c = a/b;
    else if (argv[1][0] == '>' && argv[1][1] == '\0') c = a > b;
    else if (argv[1][0] == '>' && argv[1][1] == '=') c = a >= b;
    else if (argv[1][0] == '<' && argv[1][1] == '\0') c = a < b;
    else if (argv[1][0] == '<' && argv[1][1] == '=') c = a <= b;
    else if (argv[1][0] == '=' && argv[1][1] == '=') c = a == b;
    else if (argv[1][0] == '!' && argv[1][1] == '=') c = a != b;
...
int picolCommandSet(struct picolInterp *interp, int argc, char **argv, void *pd)
{
    ...
    if (argc == 3)
    {
        ...
        picolSetResult(interp,argv[2]);
    }
    else if (argc == 4)
    {
        v = picolGetVar(interp, argv[1]);
        value = strtoul(v->val, NULL, 0);
        p = malloc(sizeof(char)*32);
        for (i=0; argv[2][i]!=':'; j++,i++)
        {
            p[j] = argv[2][i];
        }
        p[i] = '\0';
        start = (int)strtoul(&p[1], NULL, 0);
        for (j=0,i++; argv[2][i]!=')'; j++,i++)
        {
            p[j] = argv[2][i];
        }
        p[j] = '\0';
        end = (int)strtoul(p, NULL, 0);
        if (start > end)
        {
            i = start;
            start = end;
    ...
    {
        return picolArityErr(interp,argv[0]);
    }
    return PICOL_OK;
}
...
int picolCommandGet(struct picolInterp *i, int argc, char **argv, void *pd)
{
    ...
    v = picolGetVar(i, argv[0]);
    value = strtoul(v->val, NULL, 0);
    for (j=0; argv[1][k]!=':'; j++,k++)
    {
        p[j] = argv[1][k];
    }
    p[j] = '\0';
    start = (int)strtoul(&p[1], NULL, 0);
    for (j=0,k++; argv[1][k]!=')'; j++,k++)
    {
        p[j] = argv[1][k];
    }
    p[j] = '\0';
...
/* r8命令（定义变量）实现函数 */
int picolCommandr8(struct picolInterp *i, int argc, char **argv, void *pd) 
{
    if (argc != 2) return picolArityErr(i,argv[0]);
    int a = atoi(argv[1]);
    uint8 d = Read8((uint8)a);
    printf("%d\n", d);
    return PICOL_OK;
}

int picolCommandw8(struct picolInterp *i, int argc, char **argv, void *pd) 
{
    if (argc != 3) return picolArityErr(i,argv[0]);
    int a = atoi(argv[1]);
    int d = atoi(argv[2]);
    Write8((uint8)a, (uint8)d);
    return PICOL_OK;
}
void picolRegisterCoreCommands(struct picolInterp *i) {
    ...
    picolRegisterCommand(i,"get",picolCommandGet,NULL);
...
int main(int argc, char **argv) 
{
    ...
    home = getenv("HOME");
    dir = (char*)malloc((80 * sizeof(char)));
    strcpy(dir, home);
    strcat(dir, "/");
